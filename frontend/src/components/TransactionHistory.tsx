...
export const TransactionHistory = ({ transactions, customer }: TransactionHistoryProps) => {
  const { user } = useAuth();
  const navigate = useNavigate();
  const title = customer ? `${customer.name}'s Transactions` : 'All Transactions';
  ...
  const groupedTransactions = customer ? null : transactions.reduce((acc, transaction) => {
  ...
  const customerSummaries = groupedTransactions ? Object.entries(groupedTransactions).map(([customerName, customerTransactions]) => {
    ...
    const latestTransaction = customerTransactions.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())[0];
  ...
  }).sort((a, b) => new Date(b.latestDate).getTime() - new Date(a.latestDate).getTime()) : null;
            ...
            {customerSummaries!.map((summary) => (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    <p className="font-medium text-sm">{summary.customerName}</p>
                ...
                <div className="text-right">
                  <p className={`font-semibold ${
                    summary.netAmount > 0 ? 'text-warning' : summary.netAmount < 0 ? 'text-success' : 'text-muted-foreground'
                  }`}>
                    {summary.netAmount > 0 ? '+' : summary.netAmount < 0 ? '-' : ''}₹{Math.abs(summary.netAmount)}
                  </p>
                  <p className="text-xs text-muted-foreground">
